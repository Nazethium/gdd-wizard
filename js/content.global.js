...
(function(){
  ...
  // Parse Markdown to HTML (for user content in PDFs)
  function parseMarkdown(s)
  {
    if (!s) return '';
    // marked.parse() supports full Markdown including newlines, lists, headers, etc.
    if (typeof marked !== 'undefined' && marked.parse) {
        // Remove wrapping <p> tags if the content is simple single-line text
        const html = marked.parse(String(s), { breaks: true });
        return html.trim();
    }
    return escapeHtml(s); // Fallback if marked.js fails to load
  }
  ...
  // ---------- i18n-friendly paragraph helper ----------
    function P3Key(titleKey, value){ return `<h3>${escapeHtml(titleKey)}</h3><p>${parseMarkdown(value||'')}</p>`; }
  ...
  function renderMarketing(d){
    const P = (k, v) => `<h3>${escapeHtml(k)}</h3><p>${parseMarkdown(v||'')}</p>`;
    ...
    const audienceBlock = (primary || secondary)
        ? [
            primary ? `<p><strong class="label">export.primaryAudience</strong> ${parseMarkdown(primary)}</p>` : '',
            secondary ? `<p><strong class="label">export.secondaryAudience</strong> ${parseMarkdown(secondary)}</p>` : ''
        ].join('')
      : '<p>—</p>';
    return [
  ...
  function renderFeatures(d){
    ...
    return feats.map(f => {
        const title = parseMarkdown(f.title||'export.untitledFeature');
        const purpose = String(f.purpose||'').trim();
        const impact = String(f.impact||'').trim();
        const rows = [];
        if (purpose) rows.push(`<p><strong class="label">export.purpose</strong> ${parseMarkdown(purpose)}</p>`);
        if (impact)  rows.push(`<p><strong class="label">export.playerImpact</strong> ${parseMarkdown(impact)}</p>`);
